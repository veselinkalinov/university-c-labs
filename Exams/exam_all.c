...
Book *addBook(Book *arr, int *count)
{
    ...
    fprintf(ft, "%d;%49s;%6s;%d;%.2f", (int)strlen(b->title), b->title, b->id, b->year, b->price);
...
{
  int n = 0;
  printf("Books after %d:\n", year);
  for (int i = 0; i < count; i++) {
    if (arr[i].year > year)
    {
      n++;
      printf("%49s - %6s - %d - %.2f", arr[i].title, arr[i].id, arr[i].year, arr[i].price);
    }
    }
    if(n == 0)
    {
...
int main(void)
{
    ...
    printf("Count of books after %d: %d", year,n);

    // Zad 3:
    char id[7];
    printf("Search id (example id: A1234B): "); scanf("%6s", id);
